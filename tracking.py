...
def create_wm_mask(aparc):
    ...
    print("Created wm mask :" (end-start))
    return wm_mask
...
def create_CSA_model(gtab):
    ...
    print('Creating CSA Model: ' (end-start))
    return csa_model

def create_CSA_peaks(csa_model, dmri, wm_mask):
...
def create_wm_seeds(wm_mask, affine):
    ...
    print('Created White Matter seeds: ' (end-start))
    return seeds
...
def compute_streamlines(peaks, classifier, seeds, affine):
    ...
    print("Computed streamlines " + (end-start))
    return streamlines
...
def create_CSD_model(gtab, dmri, wm_mask):
...
def prob_direction_getter(csd_fit):
    ...
    print('Created the Direction Getter: ' (end-start))
    return prob_dg
...
def main(det):
    ...
    if det:
        ...
        csa_peaks = create_CSA_peaks(csa_model, dmri, wm_mask)
        ...
        # Compute streamlines
        streamlines = compute_streamlines(csa_peaks, affine, classifier, seeds)
        ...
        # Create CSD model
        csd_fit = create_CSD_model(gtab, dmri, wm_mask)

        # Create the probabailistic direction getter
        prob_dg = prob_direction_getter(csd_fit)

        # Create classifier
        classifier = create_classifier(prob_dg)

        # Create seeds
        seeds = create_wm_seeds(wm_mask, affine)

        # Compute streamlines
        streamlines = compute_streamlines(prob_dg, affine, classifier, seeds)
